...
router.post("/", async (req: Request, res: Response) => {
  try {
    const { id, compId, meta, referralCode, roundId } = req.body;
    ...
    // round, enforce its gating rule, and use the round's fee (not the
    // competition fee) for the status decision.
    let effectiveFee: number = compResult.rows[0].fee;
    if (roundId) {
      const roundRes = await pool.query(
        "SELECT fee FROM competition_rounds WHERE id = $1 AND comp_id = $2",
        [roundId, compId]
      );
      if (roundRes.rows.length === 0) {
        res.status(400).json({ message: "Round not found for this competition" });
        return;
      }
      effectiveFee = Number(roundRes.rows[0].fee) || 0;
      const gate = await checkRoundGating(pool, req.userId!, roundId);
      if (!gate.allowed) {
        res.status(403).json({ message: gate.reason ?? "This round is not available yet." });
        return;
      }
    }
    ...
    const insertResult = await pool.query(
      `INSERT INTO registrations (id, user_id, comp_id, round_id, status, meta, profile_snapshot, referral_code)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING registration_number`,
      [id, req.userId, compId, roundId ?? null, initialStatus, meta ? JSON.stringify(meta) : null, JSON.stringify(profileSnapshot), referralCode ?? null]
    );
    ...
    if (err.code === "23505") {
      const msg = err.constraint === "uq_registrations_user_comp_round"
        ? "You have already registered for this round."
